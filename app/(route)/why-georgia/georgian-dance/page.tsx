...
const Page: React.FC = () => {
  ...
  const section01 = sections9.find((section) => section.id === "section-01");
  ...
  const contentRefs = useRef<(HTMLDivElement | null)[]>(
    new Array(contentItems9.length).fill(null)
  );

  const handleScrollToSection = (title: string) => {
    const index = contentItems9.findIndex((item) => item.title === title);
    ...
    <>
      ...
      <div className="container mx-auto px-6 sm:px-8 md:px-10 lg:px-11 pt-12 sm:pt-12 grid grid-cols-1 sm:grid-cols-4 gap-4 sm:gap-5 min-h-screen">
        ...
        <div className="col-span-1 sm:col-span-3 mt-6 sm:mt-0">
          <ParagraphText
            anotherDescription={contentItems9[0].anotherDescription}
          />
          {contentItems9.map((item, index) => (
            <motion.div
              key={index}
              ref={(el) => {
